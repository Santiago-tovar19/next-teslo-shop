...
export const paypalCheckPayment = async (transactionId: string) => {
  ...
  if (!authToken)
    return { ok: false, message: "No se pudo obtener el token de autenticación de PayPal" };
  ...
  if (status !== "COMPLETED") {
    return { ok: false, message: "El pago no ha sido completado" };
  }

  try {
    await prisma.order.update({
      where: { id: orderId },
      data: { isPais: true, paisAt: new Date() },
    });
